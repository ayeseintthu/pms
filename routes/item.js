...
router.get("/add",checkAccount, async function(req,res){
    ...
    res.render("items/index",{classes: classes});
});
...
router.get("/list", checkAccount, async function(req,res){
  const items = await Item.find({status: true}).populate("classId","name").populate("categoryId", "category").populate("createdBy","name").populate("updatedBy","name");
  const classes = await Classes.find({status: true});
  console.log(items);
  res.render("items/list",{items:items, classes: classes});
